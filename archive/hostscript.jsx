...
function setMDTag(layer, tag) {
    ...
    if (!tagEffect) {
        // Step 1: Add the Dropdown Menu Control
        tagEffect = fx.addProperty('Dropdown Menu Control');
        if (!tagEffect) throw new Error('addProperty returned null on "' + layer.name + '".');
        
        alert('Step 1: Created Dropdown Menu Control on "' + layer.name + '"');

        // Step 2: Rename to 'MDKit Tag'
        tagEffect.name = MDKIT_TAG_EFFECT_NAME;
        alert('Step 2: Renamed to "' + MDKIT_TAG_EFFECT_NAME + '"');

        // Step 3: Use setPropertyParameters to set the 4 items
        // Re-fetch to get fresh reference after rename
        tagEffect = fx.property(MDKIT_TAG_EFFECT_NAME);
        var menuProp = tagEffect.property(1);
        
        alert('Step 3: Using setPropertyParameters to set items');
        try {
            menuProp.setPropertyParameters(['Overlay', 'VO', 'SFX', 'Music']);
            alert('Step 3: setPropertyParameters succeeded');
        } catch (e) {
            alert('Step 3: setPropertyParameters failed: ' + e.toString());
        }

        // Step 4: Set to the correct tag index
        alert('Step 4: About to setValue(' + idx + ') on ' + tag);
        menuProp.setValue(idx);
        alert('Step 4: setValue(' + idx + ') completed. Current value: ' + menuProp.value);

    } else {
        // Effect already exists — just update the value
        alert('MDKit Tag already exists on "' + layer.name + '". Setting to index ' + idx);
        tagEffect.property(1).setValue(idx);
        alert('Updated to index ' + idx + '. Current value: ' + tagEffect.property(1).value);
    }
}
