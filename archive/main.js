...
(function () {
    ...
    var applyTagBtn = document.getElementById('applyTagBtn');
    var clearTagBtn = document.getElementById('tagClearBtn');
    ...
    if (applyTagBtn && tagDropdown) {
        applyTagBtn.addEventListener('click', function () {
            var selectedIndex = parseInt(tagDropdown.value, 10);
            var tagNames = { 1: 'Overlay', 2: 'VO', 3: 'SFX', 4: 'Music' };
            var tagName = tagNames[selectedIndex] || 'Tag';
            
            setStatus('Applying ' + tagName + ' tag (index ' + selectedIndex + ')…', true);
            callHost(
                'applyTagByIndex',
                [selectedIndex],
                function (res) {
