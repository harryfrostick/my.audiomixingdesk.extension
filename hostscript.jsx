...
function applyTagByIndex(idx) {
    try {
        ...
        for (var i = 0; i < toTag.length; i++) {
            ...
            try {
                var fx = layer.property('Effects');
                if (!fx) throw new Error('No Effects');

                // Remove any existing MDKit Tag
                var existing = fx.property('MDKit Tag');
                if (existing) existing.remove();

                // Apply the pseudo effect using its match name
                var tagEffect = fx.addProperty('Pseudo/MDK-Tag');
                if (!tagEffect) throw new Error('Cannot add MDKit Tag pseudo effect');

                // Explicitly set the name to "MDKit Tag"
                tagEffect.name = 'MDKit Tag';

                // Set the selected index (1-4)
                var menuProp = tagEffect.property(1);
                if (!menuProp) throw new Error('Cannot access menu property');
        ...
        var msg = 'Tagged ' + count + ' layer(s) with MDKit Tag set to ' + (tagNames[idx] || idx) + '.';
        return _ok(msg);

    } catch (e) {
        try { app.endUndoGroup(); } catch (x) {}
        return _err('applyTagByIndex: ' + e.toString());
    }
}
